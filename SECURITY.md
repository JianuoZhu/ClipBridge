...
ClipBridge（界面名称 Jianuo Clip）面向单个所有者和少量可信设备，使用共享 PIN、独立管理员账号和一个服务实例。
...
- PIN 和管理员密码用随机盐和 scrypt 派生后验证；数据库保存组合凭据的派生状态，不保存明文凭据。默认 PIN `1223` 是公开示例，部署时应修改。短 PIN 的强度低于随机密码。
- PIN 会话只允许访问共享传输；文件库所有列表、预览、下载、上传、编辑和删除接口都要求服务端 `admin` 角色。未配置管理员密码时禁止管理员登录。
...
- 上传文件在 Web 根目录外用随机名称保存，下载强制作为附件；大小与共用文件配额在接收过程中检查。管理员图片预览验证 PNG/JPEG/GIF/WebP 签名，只允许这些类型内联显示；文本以 JSON 返回并用文本框显示，不执行 HTML/SVG 或脚本。
- 文本编辑采用新文件写入与版本检查，冲突不覆盖原文件；文件库不会自动过期。
- 登录按应用看到的连接 IP 和入口分别限速，每 15 分钟最多累计 5 次未成功尝试，成功登录清除该入口计数；同时最多进行 4 次验证。计数保存在进程内，重启后清空。
...
- 首次升级到 PIN 与管理员角色版本时，旧会话也会撤销。凭据不变的后续重启保留有效会话。
